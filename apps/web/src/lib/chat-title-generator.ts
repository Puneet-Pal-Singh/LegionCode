...
const ACTION_REPLACEMENTS: Array<[RegExp, string]> = [
  [
    /\bcheck\s+(?:my|our)?\s+(.+?)\s+and\s+make\s+it\s+pretty\b/i,
    "improve $1",
  ],
  [
    /\b(?:yes\s+)?lets?\s+make\s+(?:my\s+)?(.+?)\s+(?:amazing|awesome|great)\b/i,
    "improve $1",
  ],
  [/\bmake\s+(?:my\s+)?(.+?)\s+prettier?\b/i, "improve $1"],
  [/\bmake\s+(?:my\s+)?(.+?)\s+(?:amazing|awesome|great)\b/i, "improve $1"],
