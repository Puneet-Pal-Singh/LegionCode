...
<Callout variant="warning" title="Cloud credentials are not local-only">
  Hosted cloud tasks need server-side credentials to call your selected
  provider. Do not treat cloud credentials as browser-only or local-only
  secrets.
</Callout>

The service uses plaintext credentials for provider validation, model catalog
operations that call the provider, and runtime calls for the selected provider
during an approved turn.
