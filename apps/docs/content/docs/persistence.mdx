...
The supported Cloudflare production procedure is the scheduled-only migration
worker documented in [Deployment](/docs/deployment/#deploy-order). It has no
HTTP handler or public route. Keep the main Brain Worker in
`DATABASE_MIGRATIONS_MODE=manual`; do not temporarily enable automatic
migrations on live request handlers.
