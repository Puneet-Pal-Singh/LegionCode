...
**Runtime commands hit the wrong service:** check `MUSCLE_BASE_URL` locally and compare runtime fingerprint endpoints on ports `8787` and `8788`.
...
<Callout variant="tip" title="Keep correlation identifiers">
  When reporting a runtime failure, include the relevant task, turn, or
  run-attempt identifier, correlation identifier, endpoint, status code, and
  sanitized error message. Never include tokens, provider keys, or repository
  contents.
</Callout>
