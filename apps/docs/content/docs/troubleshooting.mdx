...
Start with the part of the system that owns the failed behavior. Browser rendering belongs to the workspace UI, authentication and product access belong to Brain, lifecycle belongs to the runtime, and repository commands belong to the secure runtime.
...
**Execution is rejected:** the selected model may lack the capabilities required by the task. Choose another model or provider and retry the turn.
...
**Runtime commands hit the wrong service:** check `VITE_MUSCLE_BASE_URL` locally and compare runtime fingerprint endpoints on ports `8787` and `8788` with their respective workers.
...
<Callout variant="tip" title="Keep correlation identifiers">
  When reporting a runtime failure, include sanitized task, turn, or run-attempt
  identifier, correlation identifier, endpoint, status code, and sanitized error
  message. Never include tokens, provider keys, or repository contents.
</Callout>
