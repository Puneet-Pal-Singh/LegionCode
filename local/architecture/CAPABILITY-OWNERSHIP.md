...
| Transcript and refresh reconstruction           | Brain lifecycle append callback buffers one turn-scoped assistant transcript and persists it once at terminal settlement; TranscriptController metadata hydration | Canonical lifecycle replay plus persisted transcript identity | Web transcript hydration and SDK lifecycle replay                 | Canonical for Plan 049 | Delete any remaining activity/RunEvent transcript reconstruction consumer; the active Plan path has none |
...
| Workspace execution                             | Brain orchestration and secure cloud runtime; automatic Web workspace bootstrap and passive top-nav Git observer removed | Runtime workspace authority behind execution backend contract | Runtime tools and checkout-bound Git/artifact service; explicit user-opened live review may read status | Transitional | Establish one workspace lease/manifest and backend port; remove remaining client/bootstrap and duplicate workspace-state decisions; artifact capture must reuse the canonical secure session; never restore automatic setup/chat/top-nav Git probes |
| Cloud execution backend                         | Secure agent API cancel/execute routes and worker/runtime contracts; local and deployed container capacity are aligned with Brain's parallel-run admission contract | Cloud implementation of execution backend/worker protocol     | Runtime kernel through Brain ExecutionService | Transitional | Move every tool and interrupt path through the backend contract; delete bypasses and stubs; keep runtime instance capacity at or above the admitted parallel-run floor |
...
| Context assembly                                | Runtime context preparation boundary                                             | Runtime Context Engine                                        | Model/harness gateway; lifecycle budget projection | Canonical for Plan 049 | Context window facts come from provider/model metadata; unavailable facts remain unavailable |
...
| Provider/model integration                      | Provider services/catalog plus runtime gateway; OpenAI-compatible transports preserve their logical provider identity | Provider catalog and narrow provider/model adapter            | Runtime Context Engine, kernel, and Web provider projection | Transitional | Connected provider inventories are preloaded into picker/manage projections after bootstrap; OpenAI and Groq use their remote model endpoints; provider discovery is demand-refreshed after a one-hour TTL and supports explicit refresh. Remove provider decisions and auxiliary title inference from turn admission; transport compatibility must never rewrite logical provider identity; make capability and usage reporting adapter contracts |
