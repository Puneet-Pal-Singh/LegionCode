...
function toolEditsScenario(): GoldenLifecycleScenario {
  ...
  return scenario("prompt -> plan -> tool calls -> edits -> multi-file turn diff -> completed", [
    ...
    toolEvent(context, 8, "tool_call.started", tool, call, {
      display: { title: "shell", family: "shell", namespace: null },
    }),
    toolEvent(context, 9, "tool_call.input_delta", tool, call, {
    ...
    event(
      ...
      { detail: "Updated repository files." },
      { itemId: file },
    ),
...
function toolInterruptScenario(): GoldenLifecycleScenario {
  ...
  return scenario("interrupt during tool execution", [
    ...
    toolEvent(context, 5, "tool_call.started", tool, call, {
      display: { title: "shell", family: "shell", namespace: null },
    }),
