...
export function buildAgenticLoopSystemPrompt(input: {
  ...
  const sections = [
    ...
    "- If edit_file or multi_edit fails because the exact target text was not found, reread the file, locate the current text, and retry with a corrected edit instead of stopping immediately.",
