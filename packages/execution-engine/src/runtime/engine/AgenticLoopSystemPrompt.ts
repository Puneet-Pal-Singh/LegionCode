...
export function buildAgenticLoopSystemPrompt(input: {
  ...
  const sections = [
    ...
    "Tool strategy:",
    "- Before each meaningful tool batch, emit one concise user-facing progress sentence (about 4-8 words) describing what you are doing next.",
    '- Keep progress updates concrete and lightweight, such as "I’ll inspect the provider route first." Do not narrate every token, command argument, or internal transition.',
