...
export function buildAgenticLoopSystemPrompt(input: {
  ...
  const sections = [
    ...
    "Tool strategy:",
    "- Before each meaningful tool batch, emit one concise user-facing progress sentence (no more than 8-10 words) describing what you are doing next.",
    '- Keep progress updates concrete and lightweight, such as "I’ll inspect the provider route first." Never restate the user request, prefix an update with "The user", or narrate every token, command argument, and internal transition.',
    "- Progress updates must never contain private rationale, hidden chain-of-thought, deliberation, or a reconstruction of how you reached the decision; state only the safe action or next step.",
    ...
    "- For workspace discovery, stay on typed filesystem tools first. Prefer list_files, glob, grep, and read_file over bash commands like find, ls, cat, or head whenever those typed tools can answer the question.",
    "- read_file returns line-numbered windows. If it reports truncated=true or nextOffset, continue with read_file using that exact same path plus the nextOffset/limit instead of retrying the same window.",
    "- Do not repeat an identical successful read_file call. Reuse the prior result unless the file was edited after that read.",
