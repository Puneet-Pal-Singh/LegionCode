CREATE TABLE IF NOT EXISTS canonical_run_projections (
  ...
  CONSTRAINT canonical_run_projections_status_check
    CHECK (status IN ('queued', 'running', 'waiting_for_approval', 'completed', 'failed', 'cancelled', 'timed_out')),
  CONSTRAINT canonical_run_projections_mode_check
    CHECK (mode IN ('ask', 'auto_edit', 'review', 'plan')),
...
CREATE TABLE IF NOT EXISTS canonical_run_item_projections (
  ...
  CONSTRAINT canonical_run_item_projections_type_check
    CHECK (item_type IN ('user_message', 'assistant_message', 'reasoning_summary', 'tool_call', 'tool_result', 'approval_request', 'approval_decision', 'file_change', 'git_operation', 'artifact_reference', 'context_compaction', 'system_marker')),
...
CREATE TABLE IF NOT EXISTS canonical_approval_projections (
  ...
  CONSTRAINT canonical_approval_projections_status_check
    CHECK (status IN ('requested', 'decided')),
  CONSTRAINT canonical_approval_projections_decision_check
    CHECK (decision IS NULL OR decision IN ('approved', 'denied', 'cancelled')),
