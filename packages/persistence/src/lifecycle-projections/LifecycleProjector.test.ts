...
describe("LifecycleProjector", () => {
  it("rebuilds the same terminal projection as incremental application", () => {
    ...
    const projector = new LifecycleProjector(
      events[0]?.turnId ?? "trn_projection001",
    );
    let live = null;
  ...
  it("rejects projection gaps and post-terminal events", () => {
    ...
    expect(() =>
      projectLifecycleEvents([
        events[0] as LifecycleEvent,
        events[2] as LifecycleEvent,
      ]),
    ).toThrow(LifecycleProjectionError);
    expect(() =>
      projectLifecycleEvents([
        ...events,
        event(9, "turn.blocking_changed", { blockingState: { kind: "none" } }),
      ]),
    ).toThrow(LifecycleProjectionError);
  });
});
